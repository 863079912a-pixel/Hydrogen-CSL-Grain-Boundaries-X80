# -*- coding: utf-8 -*-
"""
run_nonequilibrium_tensile_0H_1H.py

Stage III-3 0H/1H dynamic tensile fracture script, updated to the same
execution/output framework as the refined 005/010/015/020 and multiH
025/050/100 scripts.

This version keeps the original 0H/1H dynamic tensile logic as the main line,
but adds the functions already used by the other two Stage 3-3 scripts:
  1) pre-tension preprocessing: y-vacuum + grips + constrained minimization + mobile NVT pre-equilibration;
  2) restart-based fresh / rerun / resume_crash / extend / skip_done modes;
  3) per-chunk restart and per-chunk CSV append;
  4) Fe/C connectivity-based fracture criterion, excluding H from the load-bearing network;
  5) extend-until-fracture for cases not fractured within the initial window;
  6) frame0, chunk_dumps, enhanced dump fields for OVITO/CNA post-processing;
  7) run_summary.json and global summary CSV output.

Current project choice:
  - tensile temperature is set to 10 K for numerical stability;
  - the paper can focus on CNA/lattice-structure evolution during tensile loading;
  - stress sign is not treated as a hard rejection criterion and can be handled in post-processing.
...
# ============================================================
# 0. Key settings: paths and cases
# ============================================================
def choose_existing_path(primary: str, fallback: str) -> Path:
    """Prefer primary if it exists; otherwise use fallback.
    This keeps compatibility with older 300K-named folders while making
    C:\\stage3_fracture_response1 the default project path.
    """
    p = Path(primary)
    f = Path(fallback)
    return p if p.exists() else f

# Main 0H/1H workspace on the rented Windows host.
WORKDIR = Path(r"C:\Users\Admin01\Desktop\stage3_fracture_response1")

LAMMPS_EXE = Path(r"C:\Users\Admin01\AppData\Local\LAMMPS 64-bit 10Dec2025-MSMPI with Python\bin\lmp.exe")

# The user provided the potential directory; LAMMPS pair_coeff needs the actual file.
POTENTIAL_DIR = Path(r"C:\Users\Admin01\Desktop\stage3_fracture_response1\potential")
POTENTIAL_FILE = POTENTIAL_DIR / "BOP1.poly"
# Fallback in case the potential was copied together with WORKDIR on D:.
if not POTENTIAL_FILE.exists():
    POTENTIAL_FILE = WORKDIR / "potential" / "BOP1.poly"

SIGMAS_TO_RUN = ["sigma3", "sigma5", "sigma11", "sigma17"]
COVERAGES_TO_RUN = ["0H", "1H"]

# "all" or list such as ["sigma3_GB_0H", "sigma5_GB_1H"]
CASES_TO_RUN = "all"
...
# ============================================================
# 1b. CPU / MPI parallel controls for 128-core rented CPU
# ============================================================
# This LAMMPS executable is an MSMPI build, so use Microsoft MPI mpiexec.exe.
# Do NOT use C:\Program Files\MPICH2\bin\mpiexec.exe with this build.
TOTAL_CPU_CORES = 128
RESERVED_CPU_CORES = 8

USE_MPI = True
MPIEXEC_EXE = Path(r"C:\Program Files\Microsoft MPI\Bin\mpiexec.exe")
# If Microsoft MPI later has local-host connection/firewall issues, try: MPI_EXTRA_ARGS = ["-localonly"]
MPI_EXTRA_ARGS = []
REQUIRE_MATCHING_MPI = True
ALLOW_MPIEXEC_PATH_FALLBACK = False

# 0H/1H has 8 cases by default: 4 sigmas x 2 coverages.
# 8 workers x 16 ranks/job = 128 MPI ranks, matching the rented 128-core CPU.
MAX_WORKERS = 8
MPI_RANKS_PER_JOB = 16
OMP_THREADS = 1
...
CASE_OVERRIDES = {
    # sigma3: strong twin-related GB; slow loading and GB-location validation.
    "sigma3_GB_0H": {
        "profile_note": "sigma3-0H slow: 14A grips, 0.05 A/ps single-side pull, GB fracture required",
        "grip_thickness_A": 14.0,
        "v_pull_A_per_ps": 0.05,
        "pre_eq_time_ps": 45.0,
        "pre_min_maxiter": 12000,
        ...
        "max_total_chunks": 900,
        "require_gb_fracture": True,
        "gb_location_tol_A": 11.0,
    },
    "sigma3_GB_1H": {
        "profile_note": "sigma3-1H slow: 14A grips, 0.05 A/ps single-side pull, GB fracture required",
        "grip_thickness_A": 14.0,
        "v_pull_A_per_ps": 0.05,
        "pre_eq_time_ps": 45.0,
        "pre_min_maxiter": 12000,
    ...
    # sigma5: also slowed down now; this keeps all four GBs on a comparable
    # slow-loading protocol while still preserving the same code paths.
    "sigma5_GB_0H": {
        "profile_note": "sigma5-0H slow: 14A grips, 0.05 A/ps single-side pull, GB fracture required",
        "grip_thickness_A": 14.0,
        "v_pull_A_per_ps": 0.05,
        "pre_eq_time_ps": 45.0,
        "pre_min_maxiter": 12000,
    ...
    "sigma5_GB_1H": {
        "profile_note": "sigma5-1H slow: 14A grips, 0.05 A/ps single-side pull, GB fracture required",
        "grip_thickness_A": 14.0,
        "v_pull_A_per_ps": 0.05,
        "pre_eq_time_ps": 45.0,
        "pre_min_maxiter": 12000,
    ...
    # sigma11: use the most conservative profile because it previously showed
    # strong non-GB failure tendency.
    "sigma11_GB_0H": {
        "profile_note": "sigma11-0H very slow: 16A grips, 0.04 A/ps single-side pull, GB fracture required",
        "grip_thickness_A": 16.0,
        "v_pull_A_per_ps": 0.04,
        "pre_eq_time_ps": 55.0,
        "pre_min_maxiter": 15000,
        "pre_min_maxeval": 80000,
        "n_chunks_initial": 520,
        "max_total_chunks": 1100,
        "require_gb_fracture": True,
        "gb_location_tol_A": 12.0,
    },
    "sigma11_GB_1H": {
        "profile_note": "sigma11-1H very slow: 16A grips, 0.04 A/ps single-side pull, GB fracture required",
        "grip_thickness_A": 16.0,
        "v_pull_A_per_ps": 0.04,
        "pre_eq_time_ps": 55.0,
        "pre_min_maxiter": 15000,
    ...
    # sigma17: slowed down and thickened like sigma5, with GB-location validation.
    "sigma17_GB_0H": {
        "profile_note": "sigma17-0H slow: 14A grips, 0.05 A/ps single-side pull, GB fracture required",
        "grip_thickness_A": 14.0,
        "v_pull_A_per_ps": 0.05,
        "pre_eq_time_ps": 45.0,
        "pre_min_maxiter": 12000,
    ...
    "sigma17_GB_1H": {
        "profile_note": "sigma17-1H slow: 14A grips, 0.05 A/ps single-side pull, GB fracture required",
        "grip_thickness_A": 14.0,
        "v_pull_A_per_ps": 0.05,
        "pre_eq_time_ps": 45.0,
        "pre_min_maxiter": 12000,
...
def build_cases():
    ...
    for sigma in SIGMAS_TO_RUN:
        for cov in COVERAGES_TO_RUN:
            ...
            source = WORKDIR / sigma / cov / "source" / f"{sigma}_GB_{cov}_source.lmp"
...
def main():
    ...
    if SHOW_STARTUP_CONFIG or not TERMINAL_BRIEF:
        log("=" * 88)
        log("Stage III-3 0H/1H dynamic tensile fracture test: all-GB slow-loading thick-grip version")
    ...
    summary_csv = WORKDIR / "summaries" / "phase3_dynamic_summary_0H_1H_allGB_slow_grip.csv"
